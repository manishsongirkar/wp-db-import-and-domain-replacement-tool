...
#
# Description:
#   This script serves as the main execution engine for running all test suites
#   of the WordPress Database Import Tool. It provides options for selective
#   suite execution, controlling output verbosity, specifying report formats,
#   and managing output directories. It orchestrates the execution of modular
#   test scripts and compiles the results into comprehensive reports (JSON, HTML, TEXT).
#
# Usage:
#   ./run_tests.sh [options] [test_suite]
#
# Key Features:
# - Orchestrates modular test execution (compatibility, system, WordPress, unit, validation).
# - Supports quick mode (--quick) for faster execution.
# - Generates multi-format reports (JSON, HTML, TEXT).
# - Handles command-line argument parsing and environmental overrides.
# - Tracks test results (pass/fail/skip) and calculates overall duration.
# - Cleans up temporary test session data upon completion.
#
# Options:
...
# - Standard Bash utilities (date, uname, awk, sed, grep, mktemp, rm, bc)
#
# ================================================================
...
# ===============================================
#
# Description: Removes existing report files (HTML, JSON, TEXT) from the output directory
#              to ensure a clean slate for the new test run.
#
# Parameters:
#   - $1: output_dir (The target directory for reports).
#
# Returns:
#   - 0 (Success) always.
#
# Behavior:
#   - Uses `rm -rf` to remove specific file types.
#   - Does *not* remove the `test_sessions` directory immediately.
#
cleanup_old_reports() {
    local output_dir="$1"
...
# ===============================================
#
# Description: Removes the temporary `test_sessions` directory which contains the
#              detailed, raw JSON results generated by the test framework during execution.
#
# Parameters:
#   - $1: output_dir (The parent output directory).
#
# Returns:
#   - 0 (Success) always.
#
# Behavior:
#   - Executes cleanup using `rm -rf`.
#
...
# Description: Prints informative messages (default for non-CI mode).
...
# Description: Prints success messages (default for non-CI mode).
# Parameters: $1: message
log_success() {
    if [[ "$CI_MODE" != "true" ]]; then
...
# Description: Prints verbose messages (only when --verbose is active).
# Parameters: $1: message
log_verbose() {
...
# ===============================================
#
# Description: Displays the usage instructions, available options, and test suites.
#
# Parameters:
#   - None.
#
...
# ===============================================
#
# Description: Parses command-line arguments to set global configuration variables
#              like report format, output directory, and the selected test suite.
#
# Parameters:
#   - $@: All command-line arguments.
#
# Returns:
#   - 0 (Success) if arguments are valid.
#   - 1 (Failure) if an invalid option or argument is provided.
#
# Behavior:
#   - Sets `$REPORT_FORMATS`, `$OUTPUT_DIR`, `$VERBOSE`, `$QUICK_MODE`, and `$TEST_SUITE`.
#   - Overrides settings based on environment variables (`$TEST_VERBOSE`, etc.) if present.
#
parse_arguments() {
    while [[ $# -gt 0 ]]; do
        case $1 in
...
# ===============================================
#
# Description: Verifies that the required test framework file exists and can be
#              sourced successfully into the current shell environment.
#
# Parameters:
#   - None.
#
# Returns:
#   - 0 (Success) if the framework is loaded.
#   - 1 (Failure) otherwise.
#
# Behavior:
#   - Sets the global `TEST_FRAMEWORK` variable.
#
...
# ===============================================
#
# Description: Executes a specific modular test script, captures its exit code and
#              execution time, and records the high-level result (PASS/FAIL).
#
# Parameters:
#   - $1: suite_name (Descriptive name of the suite, e.g., "Bash Versions").
#   - $2: test_script (Path to the executable script, e.g., "test_bash_versions.sh").
#   - $3: suite_description (Brief description for reporting).
#
# Returns:
#   - The exit code of the executed test script (0 for success).
#
# Behavior:
#   - Runs the script using `bash`.
#   - Suppresses output (`>/dev/null 2>&1`) unless `$VERBOSE` is true.
#   - Updates global counters (`$TOTAL_TESTS`, `$TOTAL_PASSED`, etc.) and the `$TEST_RESULTS` array.
#
run_test_suite() {
    local suite_name="$1"
    local test_script="$2"
...
# ===============================================
#
# Description: Executes the OS/Shell and Bash version compatibility test suites.
#
# Parameters:
#   - None.
#
# Returns:
#   - 0 (Success) if all run tests passed, 1 (Failure) otherwise.
#
# Behavior:
#   - Skips Bash version tests if `$QUICK_MODE` is active, unless the suite is explicitly targeted.
#
...
# ===============================================
#
# Description: Executes the system environment and resource test suite.
#
# Parameters:
#   - None.
#
# Returns:
#   - 0 (Success) if run tests passed, 1 (Failure) otherwise.
#
run_system_tests() {
...
# ===============================================
#
# Description: Executes the WordPress integration and functionality test suite.
#
# Parameters:
#   - None.
#
...
# ===============================================
#
# Description: Executes the core function unit test suite.
#
# Parameters:
#   - None.
#
...
# ===============================================
#
# Description: Executes the final tool validation tests by running the integrated
#              `wp-db-import validate` command.
#
# Parameters:
#   - None.
#
...
# ===============================================
#
# Description: Executes the selected test suites sequentially and records the total
#              execution time.
#
# Parameters:
#   - None (Relies on global `$TEST_SUITE`).
#
# Returns:
#   - The overall result (0 for success, 1 for failure, 2 for setup error).
#
execute_tests() {
    START_TIME=$(date +%s)
...
# ===============================================
#
# Description: Creates the designated output directory for reports and initiates the cleanup
#              of old report files.
#
# Parameters:
#   - None.
#
# Returns:
#   - 0 (Success) if the directory is ready.
#   - 1 (Failure) if the directory cannot be created.
#
...
# ===============================================
#
# Description: Compiles the test results and environment information into a
...
#
# Returns:
#   - Creates `test_results.json` in `$OUTPUT_DIR`.
#
# Behavior:
#   - Uses `bc -l` (if available) to calculate the success rate.
#
...
# ===============================================
#
# Description: Compiles the test results into an interactive, human-readable HTML format.
#              It includes a summary dashboard and detailed, collapsible results for each suite.
#
# Parameters:
#   - None.
#
# Returns:
#   - Creates `test_results.html` in `$OUTPUT_DIR`.
#
# Behavior:
#   - Uses a large HTML template with placeholders that are replaced by Bash string manipulation (`sed`).
#   - Retrieves individual test results from the raw session JSON files for detailed display.
#
...
# ===============================================
#
# Description: Compiles the test results into a simple, human-readable TEXT format
#              suitable for quick viewing in the terminal or logs.
#
# Parameters:
#   - None.
#
# Returns:
#   - Creates `test_results.txt` in `$OUTPUT_DIR`.
#
...
# ===============================================
#
# Description: Orchestrates the creation of the output directory and calls the
#              individual generation functions for each required report format.
#
# Parameters:
#   - None.
#
# Returns:
#   - 0 (Success) if reports are generated.
#   - 1 (Failure) if the output directory cannot be created.
#
generate_reports() {
...
# ===============================================
#
# Description: Main function to handle setup, test execution, report generation,
#              and final cleanup.
#
# Parameters:
...
#
# Returns:
#   - The exit code of the test run (0 for all passed, 1 for failures, 2 for setup/runner error).
#
# Behavior:
#   - Calls `parse_arguments`, `check_test_framework`, `execute_tests`, and `generate_reports`.
#   - Displays a final summary to the user.
#
main() {
